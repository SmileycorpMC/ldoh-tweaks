...
public class WorldGenSafehouse extends WorldGenerator {
    ...
    private void decorateHouse(World world, Random rand) {
        ...
        for (int i = 0; i < 3; i++)
            brewingStand.setInventorySlotContents(i, new ItemStack(LDOHItems.SYRINGE, 1, rand.nextInt(2) + 1));
    ...
    private void decorateTree(World world, BlockPos pos) {
        ...
        for (EnumFacing facing : EnumFacing.VALUES) {
            if (facing != EnumFacing.UP && facing != EnumFacing.DOWN)
                tree.addOrnament(facing, new ItemStack(FurnitureBlocks.FAIRY_LIGHT));
        }
    }
