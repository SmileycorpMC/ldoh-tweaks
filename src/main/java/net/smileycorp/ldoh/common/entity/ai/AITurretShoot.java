...
public class AITurretShoot extends EntityAIBase {
	...
	public void updateTask() {
		...
		RayTraceResult ray = ModUtils.rayTrace(turret.world, turret, turret.getDistance(target));
		if (ray != null) {
			if (ray.typeOfHit == RayTraceResult.Type.BLOCK) {
				...
				if (ray.entityHit == target) {
			        ...
			        if(event == null)
			        {
			            event = SoundEvent.REGISTRY.getObject(new ResourceLocation(sound));
			        }
			        if(event != null)
			        {
			            turret.world.playSound(null, turret.getPosition(), event, SoundCategory.HOSTILE, 5.0F, 0.8F + turret.world.rand.nextFloat() * 0.2F);
			        }
