...
public abstract class EntityAbstractTurret<T extends TileAbstractTurret<P>, P extends EntityAbstractTurret<T, P>> extends EntityLiving implements IEnemyMachine {
    ...
    public void onLivingUpdate() {
        ...
        if (ticksExisted % 5 == 0 & !isEnemy()) {
            ...
            if (owner == null || owner.isDead | !owner.isAddedToWorld()) {
                String uuidString = dataManager.get(OWNER_UUID);
                if (!world.isRemote && DataUtils.isValidUUID(uuidString))
                    owner = FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList().getPlayerByUUID(UUID.fromString(uuidString));
    ...
    public void onDeath(DamageSource source) {
        ...
        for (TurretUpgrade upgrade : getInstalledUpgrades())
            if (!isEnemy() || rand.nextInt(100) < 25) entityDropItem(upgrade.getItem(), 0.0f);
    ...
    public List<TurretUpgrade> getInstalledUpgrades() {
        ...
        for (int i : ModUtils.posToArray(dataManager.get(TURRET_UPGRADES)))
            if (!TurretUpgrade.isBlank(i)) upgrades.add(TurretUpgrade.get(i));
        return upgrades;
    }

    public boolean hasUpgrade(TurretUpgrade upgrade) {
        for (int i : ModUtils.posToArray(dataManager.get(TURRET_UPGRADES)))
            if (TurretUpgrade.get(i) == upgrade) return true;
        return false;
    }
    ...
    public int getUpgradeCount(TurretUpgrade upgrade) {
        ...
        int count = 0;
        for (int i : ModUtils.posToArray(dataManager.get(TURRET_UPGRADES)))
            if (TurretUpgrade.get(i) == upgrade) count++;
        return count;
    }
