...
import net.smileycorp.hundreddayz.common.HundredDayzTweaks;
...
public class EntityTFZombie extends EntityZombie {
		...
		for (EnumTFClass tfclass : EnumTFClass.values()) {
			if (tfclass.testClass(entity.getClass())) {
				...
				for (EntityEquipmentSlot slot : EntityEquipmentSlot.values()) {
					setItemStackToSlot(slot, HundredDayzTweaks.cleanTFWeapon(entity.getItemStackFromSlot(slot)));
				}
				posX = entity.posX;
				posY = entity.posY;
				posZ = entity.posZ;
	...
	protected void setEquipmentBasedOnDifficulty(DifficultyInstance difficulty) {
		...
		for (EntityEquipmentSlot slot : EntityEquipmentSlot.values()) {
			setItemStackToSlot(slot, HundredDayzTweaks.cleanTFWeapon(baseentity.getItemStackFromSlot(slot)));
		}
	}
	...
	public EntityTF2Character getCured() {
		...
		try {
			...
			for (EntityEquipmentSlot slot : EntityEquipmentSlot.values()) {
				entity.setItemStackToSlot(slot, HundredDayzTweaks.cleanTFWeapon(getItemStackFromSlot(slot)));
			}
			return entity;
		} catch (Exception e) {e.printStackTrace();}
		return null;
